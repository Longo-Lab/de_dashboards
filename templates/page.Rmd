...
imgs <- str_c(
  file.path(seurat_dir, subclass, filedate), 
  nameset,
  '{{r}}',
...
both <- merge(drug_df, geno_df, by = 'GENE', suffix = c('.drug', '.geno'))
both <- both[(p_val.drug < 0.05 & p_val.geno < 0.05)]  # not adjusted p-val

res <- cor.test(both[['avg_log2FC.drug']], both[['avg_log2FC.geno']])

sub <- sprintf('R = %.3f; p < %.3g', res$estimate[[1]], res$p.value)
gg_fig <- both %>% 
  ggplot(aes(x = avg_log2FC.drug, y = avg_log2FC.geno)) +
  geom_point(color = NA) +
  ...
  xlab(str_c('avg_log2FC', drug, sep = '.')) +
  ylab(str_c('avg_log2FC', geno, sep = '.')) +
  theme_classic()

both_sig <- both %>% 
  filter(abs(avg_log2FC.drug) > 0.5 | abs(avg_log2FC.geno) > 0.55)

ggplotly(gg_fig, tooltip = 'none') %>% 
  add_markers(
    x = both$avg_log2FC.drug, 
    y = both$avg_log2FC.geno, 
    color = I('#8073ac'),
    hovertemplate = str_c(
      '<br><b>', both$GENE, '</b>',
      '<br>', drug, ' L2FC: ', round(both$avg_log2FC.drug, 3),
      '<br>', geno, ' L2FC: ', round(both$avg_log2FC.geno, 3), 
      '<extra></extra>'
    )
  ) %>% 
  add_text(
    x = both_sig$avg_log2FC.drug,
    y = both_sig$avg_log2FC.geno,
    text = both_sig$GENE,
...
dir_enh <- results[['{{r}}']][category == 'enhancement' & direct == T][order(-(abs(avg_log2FC.geno) + abs(avg_log2FC.drug)))]
dir_sup <- results[['{{r}}']][category == 'suppression' & direct == T][order(-(abs(avg_log2FC.geno) + abs(avg_log2FC.drug)))]

indir_enh <- results[['{{r}}']][category == 'enhancement' & direct == F][order(-(abs(avg_log2FC.geno) + abs(avg_log2FC.geno_drug)))]
indir_sup <- results[['{{r}}']][category == 'suppression' & direct == F][order(-(abs(avg_log2FC.geno) + abs(avg_log2FC.geno_drug)))]

enh <- c(dir_enh, indir_enh)
sup <- c(dir_sup, indir_sup)

comp_enh <- results[['{{r}}']][category == 'compensatory enhancement'][order(-(abs(avg_log2FC.geno) + abs(avg_log2FC.drug)))]
comp_sup <- results[['{{r}}']][category == 'compensatory suppression'][order(-(abs(avg_log2FC.geno) + abs(avg_log2FC.drug)))]
...
sketch <- htmltools::withTags(table(
  ...
  thead(
    tr(
      ...
      th(rowspan = 2, 'Direct'),
      th(colspan = 4, style = 'background-color:#f3f7eb;border-bottom:none;', geno),
      th(colspan = 4, style = 'background-color:#fdf2f1;border-bottom:none;', str_c(geno, drug, sep = '_')),
      th(colspan = 4, style = 'background-color:#eef8f9;border-bottom:none;', drug),
...
results[['{{r}}']] %>% 
  ...
  formatRound(grep('avg_log2FC', names(results[['{{r}}']]), value = T), 3) %>% 
  formatSignif(grep('p_val_adj', names(results[['{{r}}']]), value = T), 3) %>% 
